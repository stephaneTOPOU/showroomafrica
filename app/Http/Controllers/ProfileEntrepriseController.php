    ...
    public function ProfileEntreprise_pays($slug_pays, $slug_categorie, $slug_souscategorie, $slug_entreprise)
    {
        ...
        if ($slug_pays == 'tg') {
            $parametres = DB::table('pays')->where('pays.id', $pays_id[0]->id)
                ...
                ->select('*')
                ->get();
            return view('frontend.tg.profileEntreprise', compact('sousCategorieNavs', 'parametres', 'Profil_entreprises', 'avis3', 'avis', 'services', 'serviceImages', 'horaires', 'galleries', 'premiums', 'basics', 'partenaires'));
        } elseif ($slug_pays == 'bf') {
            $parametres = DB::table('pays')->where('pays.id', $pays_id[0]->id)
                ->join('parametres', 'pays.id', '=', 'parametres.pays_id')
